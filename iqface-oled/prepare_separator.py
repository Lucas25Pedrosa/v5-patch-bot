...
static BOOL IQFOLEDIsFeedSeparator(UIView *view) {
    ...
    // Exact measured dark source. Once managed, keep recognizing the same view
    // while Facebook reuses/repaints the current cell.
...
static BOOL IQFOLEDUpdateFeedSeparator(UIView *view) {
    ...
    if (shouldShow) {
        ...
        // Call the exchanged selector directly so this custom color bypasses
        // the OLED setBackgroundColor replacement and preserves the saved source.
...
print("Prepared iQFaceOLED 0.1.7 with optional feed separators")
