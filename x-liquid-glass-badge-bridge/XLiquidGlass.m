...
#pragma mark - XNavigation unread badge bridge

static IMP gOrigBadgeCountsInit = NULL;
static Class gBadgeCountsInitOwner = Nil;
static BOOL gBadgeCountsSourceHooked = NO;
static BOOL gXNavBadgeLayoutHooked = NO;
static IMP gOrigXNavBadgeItemLayout = NULL;

static NSInteger gXLGNtabUnreadCount = 0;
static NSInteger gXLGDMUnreadCount = 0;
static NSInteger gXLGXChatUnreadCount = 0;
static NSInteger gXLGTotalUnreadCount = 0;
static BOOL gXLGHasUnreadCounts = NO;

static char kXLGBadgeLabelKey;

static NSInteger XLGIntegerCount(id value) {
    ...
    return MAX((NSInteger)0, count);
}

static BOOL XLGHasIntegerValue(id value) {
    return value && value != NSNull.null &&
           [value respondsToSelector:@selector(integerValue)];
}

static UIWindow *XLGBadgeActiveWindow(void) {
    UIWindow *fallback = nil;
    for (UIScene *scene in UIApplication.sharedApplication.connectedScenes) {
        if (![scene isKindOfClass:UIWindowScene.class]) continue;
...
typedef NS_ENUM(NSInteger, XLGBadgeKind) {
    XLGBadgeKindNone = 0,
    XLGBadgeKindNotifications,
    XLGBadgeKindMessages
};

static XLGBadgeKind XLGBadgeKindForItemView(UIView *itemView) {
    if (!itemView) return XLGBadgeKindNone;

    NSString *label = itemView.accessibilityLabel.lowercaseString ?: @"";

    if ([label containsString:@"notif"]) {
        return XLGBadgeKindNotifications;
    }

    if ([label containsString:@"bate-papo"] ||
        [label containsString:@"chat"] ||
        [label containsString:@"message"] ||
        [label containsString:@"mensag"]) {
        return XLGBadgeKindMessages;
    }
    ...
    for (UIView *subview in itemView.subviews ?: @[]) {
        ...
        NSString *description =
            [((UIImageView *)subview).image.description.lowercaseString copy] ?: @"";

        if ([description containsString:@"notification"]) {
            return XLGBadgeKindNotifications;
        }

        if ([description containsString:@"message"] ||
            [description containsString:@"chat"]) {
            return XLGBadgeKindMessages;
        }
    }

    return XLGBadgeKindNone;
}

static NSInteger XLGBadgeCountForKind(XLGBadgeKind kind) {
    switch (kind) {
        case XLGBadgeKindNotifications:
            return gXLGNtabUnreadCount;

        case XLGBadgeKindMessages:
            // X can use either the legacy DM pipeline or XChat. They represent
            // alternative message backends, so use the non-zero/highest source
            // rather than summing and risking a duplicate count.
            return MAX(gXLGDMUnreadCount, gXLGXChatUnreadCount);
...
static UIImageView *XLGIconViewForTabItem(UIView *itemView) {
    for (UIView *subview in itemView.subviews ?: @[]) {
...
static UILabel *XLGBadgeLabelForItem(UIView *itemView, BOOL create) {
    UILabel *label =
        objc_getAssociatedObject(itemView, &kXLGBadgeLabelKey);
...
static void XLGLayoutBadgeForItem(UIView *itemView) {
    if (!itemView) return;

    XLGBadgeKind kind = XLGBadgeKindForItemView(itemView);
    UILabel *badge = XLGBadgeLabelForItem(itemView, kind != XLGBadgeKindNone);

    if (kind == XLGBadgeKindNone || !badge) return;

    NSInteger count = gXLGHasUnreadCounts ? XLGBadgeCountForKind(kind) : 0;
    ...
    CGFloat height = 16.0;
    CGFloat width = MAX(height, ceil(textSize.width) + 8.0);

    UIImageView *iconView = XLGIconViewForTabItem(itemView);
    CGRect iconFrame =
        iconView ? iconView.frame
                 : CGRectMake((CGRectGetWidth(itemView.bounds) - 24.0) / 2.0,
                              19.0,
                              24.0,
                              24.0);

    // Match the legacy T1TabView geometry: badge starts roughly 8 pt before
    // the icon's right edge and 4 pt above its top edge.
    CGFloat x = CGRectGetMaxX(iconFrame) - 8.0;
    CGFloat y = CGRectGetMinY(iconFrame) - 4.0;

    badge.frame = CGRectIntegral(CGRectMake(x, y, width, height));
    badge.layer.cornerRadius = height / 2.0;

    [itemView bringSubviewToFront:badge];
}

static void XLGUpdateVisibleXNavigationBadges(void) {
    UIWindow *window = XLGBadgeActiveWindow();
    if (!window) return;

    Class itemClass = NSClassFromString(@"_TtC11XNavigation14TabBarItemView");
    if (!itemClass) itemClass = NSClassFromString(@"XNavigation.TabBarItemView");
    if (!itemClass) return;
    ...
    for (NSUInteger i = 0; i < queue.count && i < 4096; i++) {
        ...
        if ([view isKindOfClass:itemClass]) {
            XLGLayoutBadgeForItem(view);
        }
...
static void XLGScheduleBadgeRefresh(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        XLGUpdateVisibleXNavigationBadges();
    });
...
static id XLGBadgeCountsInit(id self,
                             SEL _cmd,
                             id ntabUnreadCountNumber,
                             id dmUnreadCountNumber,
                             id xchatUnreadCountNumber,
                             id totalUnreadCountNumber,
                             id date) {
    id result = self;

    if (gOrigBadgeCountsInit) {
        result =
            ((id (*)(id, SEL, id, id, id, id, id))gOrigBadgeCountsInit)(
                self,
                _cmd,
                ntabUnreadCountNumber,
                dmUnreadCountNumber,
                xchatUnreadCountNumber,
                totalUnreadCountNumber,
                date
            );
    }

    gXLGNtabUnreadCount = XLGIntegerCount(ntabUnreadCountNumber);
    gXLGDMUnreadCount = XLGIntegerCount(dmUnreadCountNumber);
    gXLGXChatUnreadCount = XLGIntegerCount(xchatUnreadCountNumber);
    gXLGTotalUnreadCount = XLGIntegerCount(totalUnreadCountNumber);

    gXLGHasUnreadCounts =
        XLGHasIntegerValue(ntabUnreadCountNumber) ||
        XLGHasIntegerValue(dmUnreadCountNumber) ||
        XLGHasIntegerValue(xchatUnreadCountNumber) ||
        XLGHasIntegerValue(totalUnreadCountNumber);

    NSLog(@"[XLiquidGlass] Badge source counts ntab=%ld dm=%ld xchat=%ld total=%ld",
          (long)gXLGNtabUnreadCount,
          (long)gXLGDMUnreadCount,
          (long)gXLGXChatUnreadCount,
          (long)gXLGTotalUnreadCount);

    XLGScheduleBadgeRefresh();
    return result;
}

static BOOL XLGMethodMatchesBadgeCountsInitializer(Method method) {
    if (!method) return NO;

    if (method_getNumberOfArguments(method) != 7) return NO;
    ...
    for (unsigned int index = 2; index < 7; index++) {
        char *argType = method_copyArgumentType(method, index);
        BOOL objectArg = argType && argType[0] == '@';
        if (argType) free(argType);
        if (!objectArg) return NO;
    }

    return YES;
}

static void XLGInstallBadgeCountsSourceHook(void) {
    if (gBadgeCountsSourceHooked) return;

    SEL targetSEL = NSSelectorFromString(
        @"initWithNtabUnreadCountNumber:dmUnreadCountNumber:xchatUnreadCountNumber:totalUnreadCountNumber:date:"
    );
    ...
    for (int i = 0; i < count && !gBadgeCountsSourceHooked; i++) {
        Class cls = classes[i];
        ...
        for (unsigned int j = 0; j < methodCount; j++) {
            ...
            if (method_getName(method) != targetSEL) continue;
            if (!XLGMethodMatchesBadgeCountsInitializer(method)) continue;
            ...
            gOrigBadgeCountsInit = current;
            gBadgeCountsInitOwner = cls;

            class_replaceMethod(cls,
                                targetSEL,
                                (IMP)XLGBadgeCountsInit,
                                types);

            gBadgeCountsSourceHooked =
                class_getMethodImplementation(cls, targetSEL) ==
                (IMP)XLGBadgeCountsInit;

            if (gBadgeCountsSourceHooked) {
                NSLog(@"[XLiquidGlass] Badge source hooked on %@",
...
static void XLGXNavBadgeItemLayout(id self, SEL _cmd) {
    if (gOrigXNavBadgeItemLayout) {
        ((void (*)(id, SEL))gOrigXNavBadgeItemLayout)(self, _cmd);
    }

    if ([self isKindOfClass:UIView.class]) {
        XLGLayoutBadgeForItem((UIView *)self);
    }
}

static void XLGInstallXNavigationBadgeRenderer(void) {
    if (gXNavBadgeLayoutHooked) return;

    Class cls = NSClassFromString(@"_TtC11XNavigation14TabBarItemView");
    if (!cls) cls = NSClassFromString(@"XNavigation.TabBarItemView");
    if (!cls) return;

    gXNavBadgeLayoutHooked =
        XLGHookMethod(cls,
                      @selector(layoutSubviews),
                      NO,
                      (IMP)XLGXNavBadgeItemLayout,
                      &gOrigXNavBadgeItemLayout);

    if (gXNavBadgeLayoutHooked) {
        NSLog(@"[XLiquidGlass] XNavigation badge renderer installed");
        XLGScheduleBadgeRefresh();
    }
}

static void XLGInstallUnreadBadgeBridge(void) {
    XLGInstallBadgeCountsSourceHook();
    XLGInstallXNavigationBadgeRenderer();
}
...
static void XLiquidGlassInit(void) {
    @autoreleasepool {
        NSLog(@"[XLiquidGlass] 1.4.1 experimental XNavigation unread badge bridge loaded");
